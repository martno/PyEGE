...
Image::Image(const char* filename) {
	...
	int newWidth;
	int newHeight;
	loadImage(filename, &imageWidth, &imageHeight, &newWidth, &newHeight);
...
Image::Image(char* text, int size)
{
	...
	GLFT_Font* glftFont = new GLFT_Font("C:\\Windows\\Fonts\\times.ttf", size);
...
void Image::drawSubImg(Image *subImg, double x, double y, bool bilinear, double scaleX = 1.0, double scaleY = 1.0,
	...
	windowHandler->bindFBO(texName, imageWidth, imageHeight);
		subImg->draw(x, y, 0, 0, -1, -1, bilinear, scaleX, scaleY, rotate, rotatePtX, rotatePtY, color);
	windowHandler->releaseFBO();
}
